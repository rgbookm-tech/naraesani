...
const MultipleChoiceSentenceActivityComponent = ({ activity }: MultipleChoiceSentenceActivityProps): JSX.Element => {
  ...
  const handleChoiceClick = (itemIndex: number, choice: string) => {
    ...
    const numBlanks = getBlanks(activity.items[itemIndex].sentence).length;
    ...
    if (nextBlankIndex === numBlanks) {
      const isAnswerCorrect = newUserAnswers[itemIndex].every(
        (answer, index) => answer === activity.items[itemIndex].correctAnswer[index]
      );
  ...
  const renderSentenceWithBlanks = (itemIndex: number) => {
    const item = activity.items[itemIndex];
    const sentence = item.sentence;
    const answers = userAnswers[itemIndex];
    const currentBlankIndex = currentBlankIndices[itemIndex];
    const itemIsCorrect = isCorrect[itemIndex];
              ...
              <div className="flex flex-wrap gap-4 justify-center mt-4">
                {item.choices.map((choice) => (
                  <button
                    ...
                    className="px-8 py-3 text-2xl font-semibold bg-white rounded-lg border-2 border-gray-300 hover:bg-orange-100 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {choice}
                  </button>
                ))}
...
export default MultipleChoiceSentenceActivityComponent;
>>>>>>> bf50f6a (chore: add build output for GitHub Pages)
